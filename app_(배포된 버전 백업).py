...
# --- 1. 보안 및 API 키 설정 ---
# 배포 후 Streamlit Cloud의 Advanced Settings > Secrets에 GOOGLE_API_KEY를 입력하세요.
if "GOOGLE_API_KEY" in st.secrets:
    GOOGLE_API_KEY = st.secrets["GOOGLE_API_KEY"]
else:
    # 로컬 테스트 시에는 여기에 키를 임시로 넣으셔도 됩니다.
...
PROMPTS = {
    "영어": "Translate all visible text to English. Preserve exact layout and text box size. If text overflows, use shorter synonyms or abbreviations to fit original text box exactly. Do not alter non-text elements.",
    "일본어": "Translate all visible text to Japanese (Kanji/Kana). Preserve exact layout and text box size. Use Kanji compounds to save space. If text overflows, use simpler phrasing or plain form to fit original text box exactly. Do not alter non-text elements.",
...
with st.sidebar:
    ...
    for lang in PROMPTS.keys():
        if f"lang_{lang}" not in st.session_state:
            # 기본 선택값 설정 (필요 시 변경 가능)
            st.session_state[f"lang_{lang}"] = (lang in ["영어", "일본어"])
